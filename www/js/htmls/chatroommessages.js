async function initChatRoomMessages() {
    ...
    for (let i = list.length - 1; i >= 0; i--) {
        ...
        html += `<li class="${css}">
                ...
                <label class="orange-text">${list[i].member}</label>
                ...
                <label id="ipfs_check_${list[i].ipfs}" class="red-text">${list[i].check}</label>
                </p><div id="ipfs_${list[i].ipfs}">
                File <a href="https://ipfs.io/ipfs/${list[i].ipfs}" target="_blank">${list[i].ipfs}</a> loading
                </div> 
                <div id="ipfs_link_${list[i].ipfs}" style="display: none">
                or see file <a href="https://ipfs.io/ipfs/${list[i].ipfs}" target="_blank">${list[i].ipfs}</a>
                </div>             
            </li>`;
    }
    html += '</div>';
...
async function _innerIPFS(encrypt, ipfs, check) {
    let fileBuffer = await currentIPFS.node.files.cat(ipfs);
...
async function createNewChatRoomMessageModal() {
    ...
    document.getElementById('addrNewChatRoomMessageModal').innerHtml = `<a href="https://ropsten.etherscan.io/address/${currentRoom.addr}" target="_blank">${currentRoom.addr}</a>`;
...
async function saveNewChatRoomMessageModal() {
    ...
    let results = await currentIPFS.node.files.add({
        content: content
    });
    const fileHash = results[0].hash;
    const fileBuffer = await currentIPFS.node.files.cat(fileHash);
